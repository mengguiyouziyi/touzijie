...
class TouzishijianSpider(scrapy.Spider):
	...
	def get_detail(self, response):
		...
		mz_sj_create_time = li_tags[2].xpath('./text()').extract_first()
		mz_sj_state = li_tags[3].xpath('./text()').extract_first()
		tz_jg_detail_half = li_tags[4].xpath('./a/@href').extract_first()
		tz_jg_detail = urljoin(self.burl, tz_jg_detail_half)
		tz_jg_name = li_tags[4].xpath('./a/text()').extract_first()
		target_size = li_tags[5].xpath('./text()').extract_first()
		capital_type = li_tags[6].xpath('./text()').extract_first()
		mz_sj_money = li_tags[7].xpath('./text()').extract_first()
